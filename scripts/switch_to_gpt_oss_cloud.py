...
def main():
    print("=== SWITCHING ALL APPS TO gpt-oss:20b-cloud MODEL WITH STOP PARAMETERS IN DIFY DB ===")
    ...
    for app_name, info in APPS_MAP.items():
        ...
        # Model config JSON using gpt-oss:20b-cloud with stop parameters
        ...
        """
        out, err = run_sql(sql)
        print(f"  - Updated {app_name} -> Output: {out.strip()} | Error: {err.strip()}")
