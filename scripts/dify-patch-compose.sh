...
# 5. Remove oracle + iris services (not needed)
...
# 6. Remove empty networks: lines (left after milvus removal)
content = re.sub(r'^(\s+)networks:\s*\n(?!\s+\-)', '', content, flags=re.MULTILINE)

# 7. Remove oradata (was under networks: in upstream)
content = re.sub(r'^  oradata:.*', '', content, flags=re.MULTILINE)

# 8. Remove dify_es01_data volume
content = re.sub(r'^  dify_es01_data:.*', '', content, flags=re.MULTILINE)

# 9. Replace 'default' network with 'ols-chatbot'
...
# 10. Add ols-chatbot network definition
